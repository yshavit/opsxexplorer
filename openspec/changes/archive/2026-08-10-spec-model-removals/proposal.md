...
- `tui-specdiff` splits a removed requirement's removal note by line — not by markdown paragraph, since OpenSpec's own convention writes `**Reason**`/`**Migration**` as two lines sharing one markdown paragraph (no blank line between them) — and renders each line directly above the requirement's intro row, in modification (yellow/orange) styling — neither the insertion nor the deletion styling used elsewhere in the pane — following the same fits-in-one-line-or-collapsible convention already used for a requirement's intro row. A line beginning `**Reason**` or `**Migration**` has that keyword de-asterisked and styled, the same way a scenario body's `WHEN`/`THEN`/`AND` bullet keyword already is; any other line still renders, defensively, without keyword stripping.
...
- `src/tui/layout.rs`, `src/tui/diff_row.rs`: every existing match on `Operation::Removed` as a unit variant needs updating for the new field; add rendering (line splitting, `Reason`/`Migration` keyword stripping alongside the existing `WHEN`/`THEN`/`AND` handling) and cursor/selectability handling for the new removal-note line rows.
